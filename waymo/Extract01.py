...
def plot_top_view_single_pic_map(trj_in, file_index, scenario_id_in, scenario, target_left, target_right,
                                 loc_target_intersection, length, filepath,lane_turn_right_id_real=[]):
    global point_has_been_pointed
...
def plot_top_view_single_pic_map_2(trj_in,file_index,scenario_id_in,scenario,target_left, target_right,length,filepath,lane_turn_right_id_real=[]):
    fig, ax = plt.subplots(1, 2, figsize=(14, 7))
    plt.xlabel('global center x (m)', fontsize=10)
    plt.ylabel('global center y (m)', fontsize=10)
    plt.axis('square')
...
def plot_top_view_single_pic_map_3(trj_in,file_index,scenario_id_in, scenario,target_left,target_right,length,trafficlight_lane,lane_turn_right_id_real=[]):
    global point_has_been_pointed
    #plt.figure(figsize=(10, 7))
    fig, ax = plt.subplots(1,2,figsize=(14,7))
    plt.xlabel('global center x (m)', fontsize=10)
...
def get_one_direction_lane_info(right_lane_id, df_all_lan_topo_info, single_map_dict, lane_turn_left_id):
    ...
    lane_entry_direction = df_all_lan_topo_info[df_all_lan_topo_info['lane_id']==entry_lane_id]['lane_direction']  #右转车道连接的进口道的方向
    lane_exit_direction = df_all_lan_topo_info[df_all_lan_topo_info['lane_id']==exit_lane_id]['lane_direction']
    # ----------------进口道信息提取--------------------
    # 车道ID及数量
    ...
    while (lane_in_left.tolist()[0] != -1):  # 如果左侧没有车道，则为单车道
        flag = 0
        lane_in_left = df_all_lan_topo_info[df_all_lan_topo_info['lane_id'] == entry_lane_id]['left_neighbors_id']
        df_all_lan_topo_info.loc[df_all_lan_topo_info['lane_id']==entry_lane_id,'lane_direction'] = lane_entry_direction
    ...
    while (lane_out_left.tolist()[0] != -1):  # 如果左侧没有车道，则为单车道
        flag = 0
        lane_out_left = df_all_lan_topo_info[df_all_lan_topo_info['lane_id'] == exit_lane_id]['left_neighbors_id']
        df_all_lan_topo_info.loc[df_all_lan_topo_info['lane_id'] == exit_lane_id, 'lane_direction'] = lane_exit_direction
...
def get_lane_direction(lane_list,single_map_dict,type):  #得到各车道的方向  进口道和出口道的车道点不一样
    direction_dict = {}  #对每个车道方向进行标记
    x_all = []
    y_all = []
    for lane in lane_list:
        if type == 'entry':
            x1, y1 = single_map_dict[lane][-1].x, single_map_dict[lane][-1].y
        if type == 'exit':
            x1, y1 = single_map_dict[lane][0].x, single_map_dict[lane][0].y
        x_all.append(x1)
        y_all.append(y1)
    x_all.sort(reverse=False)
    y_all.sort(reverse=False)
    if len(lane_list) == 4:  #表明有四个方向
        for lane in lane_list:
            if type == 'entry':
                x1, y1 = single_map_dict[lane][-1].x, single_map_dict[lane][-1].y
            elif type == 'exit':
                x1, y1 = single_map_dict[lane][-1].x, single_map_dict[lane][-1].y
                if x1 == min(x_all):
                    direction_dict[lane] = 'W'
                elif x1 == max(x_all):
                    direction_dict[lane] = 'E'
                elif y1 == min(y_all):
                    direction_dict[lane] = 'S'
                elif y1 == max(y_all):
                    direction_dict[lane] = 'N'
...
def intersection_info_extract(df_all_lane_topo_info, single_map_dict, lane_turn_left_id_ori, lane_turn_right_id_ori,
    ...
    df_all_lane_topo_info.loc[:,'lane_funciton'] = -1
    ...
    for left_lane_id in lane_turn_left_id:  # 对于所有的左转车道
        ...
        # 标记车道功能
        df_all_lane_topo_info.loc[
            df_all_lane_topo_info['lane_id'] == entry_lane_id, 'lane_function'] = 'left'  # 本身是直行车道，功能是用于左转
    ...
    for lane_in_id in all_lane_id:
        if (lane_in_id not in lane_turn_left_id) and (lane_in_id not in lane_turn_right_id):
            if judge_lane_in_intersection(single_map_dict[lane_in_id], points_key_new_plus) == 1:
                ...
                entry_lane_id = \
                df_all_lane_topo_info[df_all_lane_topo_info['lane_id'] == lane_in_id]['entry_lanes'].iloc[0]
                exit_lane_id = \
                df_all_lane_topo_info[df_all_lane_topo_info['lane_id'] == lane_in_id]['exit_lanes'].iloc[0]
                # 记录进出口道功能
                df_all_lane_topo_info.loc[df_all_lane_topo_info['lane_id'] == entry_lane_id, 'entry_or_exit'] = 'entry'
                df_all_lane_topo_info.loc[df_all_lane_topo_info['lane_id'] == exit_lane_id, 'entry_or_exit'] = 'exit'
                # 标记车道功能
                df_all_lane_topo_info.loc[
                    df_all_lane_topo_info['lane_id'] == entry_lane_id, 'lane_function'] = 'straight'  # 本身是直行车道，功能是用于直行
    ...
    df_all_lane_topo_info.loc[:,'lane_direction']
    #lane_in_intersection = df_all_lane_topo_info[(df_all_lane_topo_info['entry_or_exit']=='inside')]['lane_id'].tolist()  #得到所有在交叉口内部的车道线
    lane_right_outside_entry = df_all_lane_topo_info[(df_all_lane_topo_info['lane_funciton']=='right') & (df_all_lane_topo_info['entry_or_exit']=='entry')]
    lane_right_outside_exit = df_all_lane_topo_info[(df_all_lane_topo_info['lane_funciton']=='right') & (df_all_lane_topo_info['entry_or_exit']=='exit')]
    lane_entry_direction = get_lane_direction(lane_right_outside_entry,single_map_dict,'entry')  #得到进口道各车道的方向
    lane_exit_direction = get_lane_direction(lane_right_outside_exit,single_map_dict,'exit')
    ...
    for one_scenario in segment_dataset:  # one_scenario 就是一个scenario
        ...
        if intersection_center_loc_list != []:
            ...
            road_edge_count, lane_count, road_line, all_element_count = plot_top_view_single_pic_map(scenario_trj, file_index,scenario_label,scenario,
                                                                                                     lane_turn_left_id,lane_turn_right_id,intersection_center_loc,
                                                                                                     length,filepath_fig_test,lane_turn_right_id_real)
...
def judge_traffic_signalphase(df_single_seg_all_scenario_lane_topo_info,dynamic_map_target_lane):
    flag = -1
    return 0
